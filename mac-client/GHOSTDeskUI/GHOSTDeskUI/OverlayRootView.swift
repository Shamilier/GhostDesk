...
struct OverlayRootView: View {
        ...
        ZStack {
            VStack(spacing: 14) {
                ...
                Spacer(minLength: 0)
                    ...
                    .onChange(of: overlay.solveTrigger) { _ in
                        Task { await handleSolveShortcutTrigger() }
                    }
    ...
    @MainActor
    private func handleSolveShortcutTrigger() async {
        if askVM.isSubmitting {
            askVM.cancelStream()
        }

        let transcript = TranscriptBuffer.shared.tail(lastSeconds: 40, maxChars: 900)
        ...
        await askVM.submit(
            ...
            overrideQuestion: Self.solveShortcutPrompt
        )

        ServerClient.shared.log("solve shortcut submitted: smart=\(smartMode), speechTail=\(transcript.count) chars")
    }
...
final class AskVM: ObservableObject {
    ...
    func submit(
        ...
        smart: Bool,
        transcript: String,
        overrideQuestion: String? = nil
    ) async {
        let q = question.trimmingCharacters(in: .whitespacesAndNewlines)
        let payloadQuestion = overrideQuestion?.trimmingCharacters(in: .whitespacesAndNewlines) ?? q
        guard !payloadQuestion.isEmpty else {
            NSLog("AskVM: skipped submit — empty payload question")
            return
        }
