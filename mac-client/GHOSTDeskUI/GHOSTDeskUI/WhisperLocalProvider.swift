...
final class WhisperLocalProvider: NSObject, ObservableObject, AudioProcessing, TranscriptionProvider {
    ...
    func pushAudioBuffer(_ buffer: AVAudioPCMBuffer, at time: CMTime, speaker: SpeakerRole) {
        // WhisperLocalProvider обрабатывает аудио самостоятельно и не использует внешние буферы.
    }
...
extension WhisperLocalProvider: SCStreamDelegate {}

// MARK: - SCStreamOutput
extension WhisperLocalProvider: SCStreamOutput {
...
// MARK: - Converters
fileprivate extension WhisperLocalProvider {
...
extension WhisperLocalProvider {
