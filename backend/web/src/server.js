...
const resolveAuthorizeReturnPath = (req, primaryCandidate) => {
  if (isSafeAuthorizePath(primaryCandidate)) {
    return primaryCandidate;
  }

  if (isSafeAuthorizePath(req.session.oauthReturnTo)) {
    return req.session.oauthReturnTo;
  }

  return buildAuthorizePathFromRequest(req.session.oauthRequest);
};
    ...
    try {
      ...
      db.run(
        ...
        function (insertErr) {
          ...
          const authorizeFallback = resolveAuthorizeReturnPath(req, oauthContinue);

          if (authorizeFallback && isSafeAuthorizePath(authorizeFallback)) {
            req.session.oauthReturnTo = null;
            return res.redirect(302, authorizeFallback);
          }
